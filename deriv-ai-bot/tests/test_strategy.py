...
def test_daily_loss_limit():
    rm = RiskManager(min_balance=1.0, max_daily_loss_pct=5.0, session_stop_loss_pct=5.0)
    rm.set_session_balance(100.0)
    rm.record_trade_result(-3.0)
    rm.record_trade_result(-2.5)
    d = rm.can_trade(94.5)
    assert not d
    # Session stop-loss is the primary daily/session loss gate
    assert "session_stop_loss" in d.reason or "daily_loss_limit" in d.reason
