...
def chart_snapshot(ticks: Sequence[Dict[str, Any]], n: int = 100) -> Dict[str, Any]:
    ...
    return {
        ...
        "ema21": ema21_v,
        "ema_bull": (
            ema8_v is not None and ema21_v is not None and ema8_v > ema21_v
        ),
        "ema_bear": (
            ema8_v is not None and ema21_v is not None and ema8_v < ema21_v
        ),
...
def rise_fall_vote(chart: Dict[str, Any]) -> Tuple[Optional[str], float, Dict[str, Any]]:
    ...
    # EMA stack
    if chart.get("ema_bull"):
        call_pts += 1.2
        notes.append("ema_bull")
    if chart.get("ema_bear"):
        put_pts += 1.2
    ...
    if rsi_v is not None:
        if rsi_v >= 55:
            call_pts += 0.8 + min(0.6, (rsi_v - 55) / 40)
            notes.append(f"rsi_up={rsi_v:.1f}")
        elif rsi_v <= 45:
            put_pts += 0.8 + min(0.6, (45 - rsi_v) / 40)
