...
# Diversified volatility portfolio (override via SYMBOLS env)
SYMBOLS = [
    s.strip()
    for s in os.getenv("SYMBOLS", "R_10,R_25,R_50,R_75,R_100").split(",")
    if s.strip()
]
...
# Max stake as % of live balance
MAX_STAKE_PCT = _env_float("MAX_STAKE_PCT", 5.0)
# Floor/ceiling stake (Deriv demo min is often 0.35)
MIN_STAKE = _env_float("MIN_STAKE", 0.35)
# Optional hard max stake in account currency (None = no hard max beyond %)
_raw_max_stake = os.getenv("MAX_STAKE")
MAX_STAKE = float(_raw_max_stake) if _raw_max_stake not in (None, "") else None
